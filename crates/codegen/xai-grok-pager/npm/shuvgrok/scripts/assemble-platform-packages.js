...
//   1. Brotli-compresses the built binary into `../shuvgrok-<platform>/bin/<bin>.br`
...
// (`@shuv1337/shuvgrok`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw shuvgrok binary is
...
// the default cargo target dirs for local testing. The env var names stay
// GROK_*-prefixed on purpose: they are part of this repo's kept-compatibility
// surface, and .github/workflows/release.yml sets them by those names.
const fs = require('fs');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `shuvgrok-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] shuvgrok-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        ...
        {
            platform: 'linux', arch: 'arm64', binName: 'shuvgrok',
