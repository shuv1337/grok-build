...
- **`UserPromptSubmit` is observe-only**: shuvgrok ignores its exit code and its stdout, so an imported prompt-validation hook silently stops blocking. Use `PreToolUse` to enforce.
